...
if 'current_logic' not in st.session_state:
    st.session_state['current_logic'] = "Quant" 
...
                   
                    # ✅ 補算 P/E（解決台塑這類抓不到問題）
                    if pd.isna(pe) and not pd.isna(price) and not pd.isna(t_eps) and t_eps != 0:
                        pe = price / t_eps
                    
                    peg = get_val('peg')
                    # ✅ 手動計算 PEG（解決 N/A）
                    if pd.isna(peg):
                        if not pd.isna(pe) and not pd.isna(eps_growth) and eps_growth != 0:
                            peg = pe / eps_growth
                        else:
                            peg = np.nan
                            
...
   cols = ['代號', '名稱', 'close_price', 'pe', 'pb', 'yield', 'roe', 'rev_growth',
'eps_growth', 'gross_margins', 'fcf_yield', 'de_ratio', 'beta', 'sharpe',
'mdd', 'implied_growth', 'peg', 'chips', 'volatility', 'priceToMA60', 'industry']
   for c in cols:
...
   required_cols = ['pe', 'pb', 'yield', 'rev_growth', 'eps_growth',
'gross_margins', 'fcf_yield', 'de_ratio', 'beta', 'sharpe', 'mdd',
'implied_growth', 'peg', 'volatility', 'roe', 'priceToMA60']
...
                extra_codes = greedy_mpt_optimization(remaining_pool, returns_df, target_n=10-len(final_codes), metric_col='sharpe', initial_selected=final_codes, target_vol_max=target_vol_max)
...
                        w = get_numeric_weight(row, regime)
                        return f"{w*100:.1f}%"
                        
...
                            initial_weights = raw_w / np.sum(raw_w)
                            initial_port_variance = np.dot(initial_weights.T, np.dot(cov_matrix.loc[valid_codes, valid_codes], initial_weights))
                            portfolio_volatility = np.sqrt(initial_port_variance)
                            vol_multiplier = min(1.0, (target_vol_max / portfolio_volatility))
                            weights = initial_weights * vol_multiplier
