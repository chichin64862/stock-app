...
def get_tw_stock_list():
    ...
    twse_ind_map = {
        ...
        "34": "電子商務業", "35": "綠能環保", "36": "數位雲端", "37": "運動休閒", "38": "居家生活", "80": "管理股票",
        "1": "水泥工業", "2": "食品工業", "3": "塑膠工業", "4": "紡織纖維", "5": "電機機械",
        "6": "電器電纜", "7": "化學工業", "8": "玻璃陶瓷", "9": "造紙工業"
    }
    
    # ✅ 修復興櫃缺失：補上 _R 網址以確保完整納入
...
def fetch_global_market_data():
    ...
    # ✅ 修復興櫃缺失：補上 _R 網址以確保完整納入 PE, PB, Yield
...
def get_stock_full_optimized(stock_str, global_twse, global_rev, mkt_ret):
    ...
    eps_growth_val = info.get("earningsGrowth") if 'info' in locals() else None
    if eps_growth_val is not None:
        data["eps_growth"] = eps_growth_val * 100

    data["peg"] = calc_peg(data["pe"], data["eps_growth"])
    
    # ✅ 修復 PEG N/A: 確保當 calc_peg 失敗時，利用可靠數據補回 PEG
    if pd.isna(data["peg"]) and pd.notna(data["pe"]) and pd.notna(data["eps_growth"]) and data["eps_growth"] != 0:
...
<span style='font-weight: 700; font-size: 0.95rem;'>{dcf_status}</span>
        ...
        if current_logic == "Quant":
            ...
            is_bull = "多頭" in regime
            # ✅ 依使用者指示：動態調整目標波動率
            target_vol_max = 0.18 if is_bull else 0.12
            ...
            if len(port_df) > 0:
                ...
                # 為了確保前後連動，預先計算 Volatility Targeting 的最終權重
                ...
                if not cov_matrix.empty and len(valid_codes) == len(port_codes) and len(valid_codes) > 1:
                    ...
                    # ✅ 實作使用者指定的進階波動率縮放 (Volatility Targeting)
                ...
                with c1:
                    ...
                    port_display['建議權重'] = port_display['代號'].apply(lambda x: f"{weight_map.get(x, 0)*100:.1f}%")
                    st.dataframe(
                ...
                with c2:
                    ...
                    <div style='background-color:#1E293B; padding:15px; border-radius:4px; border:1px solid #334155;'>
                        ...
                        <div style='font-size:0.8rem; color:#64748B; margin-bottom:12px;'>* 已啟動 Volatility Targeting 控制</div>
                        <div style='display:flex; justify-content:space-between; margin-bottom:8px;'>
