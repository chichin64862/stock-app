...
def get_stock_data(symbol):
    try:
        ...
        # ✅ 精準修復：嚴格計算真實 EPS YoY (不硬湊數據，不夠 5 季就回傳 N/A)
        eps_g = np.nan
        try:
            fin = ticker.quarterly_financials
            bal = ticker.quarterly_balance_sheet
            if fin is not None and bal is not None:
                calc_res = calc_eps_yoy(fin, bal)
                if not pd.isna(calc_res):
                    eps_g = calc_res / 100.0  # 轉換為小數以相容舊版結構
        except: pass
        ...
        data = {
            ...
            'eps_growth': eps_g, # 全面棄用 Yahoo 錯誤的 earningsGrowth，改用嚴格推算的 YoY
...
# ✅ 精準修復：將 QoQ (iloc[0]-iloc[1]) 修正為真正的 YoY 年增率 (iloc[0]-iloc[4])
def calc_eps_yoy(fin, bal):
    try:
        if "Net Income" in fin.index and "Ordinary Shares Number" in bal.index:
            ...
            eps = eps.dropna()
            # 嚴格把關：必須湊齊 5 個季度才能算最新一季與去年同期的 YoY，避免硬湊數據
