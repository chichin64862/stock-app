...
# =========================================================================
# 💡 核心數據引擎：三維聚合架構 (證交所 + 玩股網 + Yahoo)
# =========================================================================

# 【模組 A】官方板塊分類庫 (快取一天，避免頻繁呼叫)
...
# 【模組 B】官方估值庫 (快取5分鐘：提供 PE, PB, Yield 備援)
@st.cache_data(ttl=300)
def fetch_twse_valuation():
    val_map = {}
    try:
        res1 = requests.get("https://openapi.twse.com.tw/v1/opendata/t187ap14_L", timeout=10, verify=False)
        if res1.status_code == 200:
            for item in res1.json():
                ...
                yld = pd.to_numeric(item.get('殖利率(%)', ''), errors='coerce')
                val_map[code] = {'pe': pe if pe > 0 else np.nan, 'pb': pb, 'yield': yld / 100.0 if not pd.isna(yld) else np.nan}
    except: pass
    try:
...
# 【模組 C】個股深度掃描 (還原無污染的 yfinance + 玩股網/證交所 聚合救援)
def get_stock_data(symbol, twse_val_map):
    try:
        raw_code = symbol.split('.')[0]
        if not symbol.endswith('.TW') and not symbol.endswith('.TWO'): symbol += '.TW'
        
        # 1. 恢復最原始的 yfinance 呼叫，絕不塞入自訂 session 破壞 Cookie！
        ...
        # --- 救援行動 1：證交所補齊 PE, PB, Yield ---
        if raw_code in twse_val_map:
            official = twse_val_map[raw_code]
            if pd.isna(pe) or pe is None: pe = official.get('pe')
            if pd.isna(pb) or pb is None: pb = official.get('pb')
            if pd.isna(yld) or yld is None: yld = official.get('yield')
            
        # --- 救援行動 2：玩股網 (WantGoo) 補齊 ROE, 毛利率 ---
        # 針對 Yahoo 封鎖最嚴重的兩項指標，啟動玩股網 API 直連
        if pd.isna(roe) or pd.isna(gross_margins) or roe is None:
            try:
                headers = {'User-Agent': 'Mozilla/5.0', 'X-Requested-With': 'XMLHttpRequest'}
                res_wg = requests.get(f"https://www.wantgoo.com/investrue/api/v1/stock/{raw_code}/financial-ratios", headers=headers, timeout=5)
                if res_wg.status_code == 200:
                    wg_data = res_wg.json()
                    if len(wg_data) > 0:
                        latest = wg_data[0] # 最新一季財報
                        if pd.isna(roe) or roe is None: roe = latest.get('returnOnEquity', 0) / 100.0
                        if pd.isna(gross_margins) or gross_margins is None: gross_margins = latest.get('grossMargin', 0) / 100.0
            except: pass
...
# =========================================================================
# 🔄 【核心修復】徹底廢除快取 (@st.cache_data)，保證每次都是最新即時搜尋！
# =========================================================================
def batch_scan_stocks(stock_list):
    ...
    # 預先載入證交所估值表供各執行緒使用
    twse_val_map = fetch_twse_valuation()
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        future_to_stock = {executor.submit(get_stock_data, s.split(' ')[0], twse_val_map): s for s in stock_list}
...
with col1:
    ...
    st.caption(f"STATUS: ONLINE | ENGINE: **{logic_badge}** | ALGO: 多源數據聚合救援版 (Yahoo + WantGoo + TWSE)")
